...
class SetPlan extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      gstIncluded: 'yes',
      discountRows: {
        room_rent: { enabled: "no", percent: "" },
        cultural_fees: { enabled: "no", percent: "" },
        caution_deposit: { enabled: "no", percent: "" },
        addmission_fee: { enabled: "no", percent: "" },
        admisson_kit: { enabled: "no", percent: "" },
        meal_price_t1: { enabled: "no", percent: "" },
        laundry_t1: { enabled: "no", percent: "" },
        meal_price_t2: { enabled: "no", percent: "" },
        laundry_t2: { enabled: "no", percent: "" },
      },
      monthly_mess_fee: 0,
      monthly_laundry_fee: 0,
      monthly_room_rent: 0,
  ...
  getDiscountConfig = (field) => {
    return (this.state.discountRows && this.state.discountRows[field]) ? this.state.discountRows[field] : { enabled: "no", percent: "" };
  };

  setDiscountConfig = (field, nextConfig) => {
    this.setState((prevState) => ({
      discountRows: {
        ...prevState.discountRows,
        [field]: {
          ...(prevState.discountRows && prevState.discountRows[field] ? prevState.discountRows[field] : { enabled: "no", percent: "" }),
          ...nextConfig,
        },
      },
    }));
  ...
  getDiscountAmount = (field, amount) => {
    const config = this.getDiscountConfig(field);
    const baseAmount = Number(amount || 0);
    const percent = Number(config.percent || 0);

    if (config.enabled !== "yes" || !percent) {
      return baseAmount;
    }

    return Number((baseAmount - (baseAmount * percent) / 100).toFixed(2));
  };

  getAdjustedFeeDetails = (item = {}) => {

    const oneTime = {
      room_rent: this.getDiscountAmount("room_rent", item.room_rent),
      cultural_fees: this.getDiscountAmount("cultural_fees", item.cultural_fees),
      caution_deposit: this.getDiscountAmount("caution_deposit", item.caution_deposit),
      addmission_fee: this.getDiscountAmount("addmission_fee", item.addmission_fee),
      admisson_kit: this.getDiscountAmount("admisson_kit", item.admisson_kit),
    };

    const termI = {
      meal_price_t1: this.getDiscountAmount("meal_price_t1", this.state.termI && this.state.termI.meal_price_t1 ? this.state.termI.meal_price_t1 : 0),
      laundry_t1: this.getDiscountAmount("laundry_t1", this.state.termI && this.state.termI.laundry_t1 ? this.state.termI.laundry_t1 : 0),
    };

    const termII = {
      meal_price_t2: this.getDiscountAmount("meal_price_t2", this.state.termII && this.state.termII.meal_price_t2 ? this.state.termII.meal_price_t2 : 0),
      laundry_t2: this.getDiscountAmount("laundry_t2", this.state.termII && this.state.termII.laundry_t2 ? this.state.termII.laundry_t2 : 0),
    };

    const oneTimeTotal =
      Number(oneTime.room_rent || 0) +
      Number(oneTime.cultural_fees || 0) +
      Number(oneTime.caution_deposit || 0) +
      Number(oneTime.addmission_fee || 0) +
      Number(oneTime.admisson_kit || 0);

    const termITotal =
      Number(termI.meal_price_t1 || 0) +
      Number(termI.laundry_t1 || 0);

    const termIITotal =
      Number(termII.meal_price_t2 || 0) +
      Number(termII.laundry_t2 || 0);

    let total_price = 0;
    if (this.state.formValues.term == "1") {
      total_price = oneTimeTotal + termITotal + termIITotal;
    } else if (this.state.formValues.term == "2") {
      total_price = oneTimeTotal + termITotal;
    } else if (this.state.formValues.term == "3") {
      total_price = oneTimeTotal + termIITotal;
    } else if (this.state.formValues.term == "4") {
      total_price = termIITotal;
    }
  ...
  renderDiscountToggle = (field) => {
    const config = this.getDiscountConfig(field);
    return (
      <div className="mt-1" style={{ fontSize: "12px" }}>
        <div className="d-flex align-items-center flex-wrap" style={{ gap: "12px" }}>
          <span style={{ fontWeight: 600 }}>Any Discount?</span>
          <label className="mb-0">
            <input
              type="radio"
              name={`discount_${field}`}
              checked={config.enabled === "yes"}
              onChange={() => this.setDiscountConfig(field, { enabled: "yes" })}
            />{" "}
            Yes
          </label>
          <label className="mb-0">
            <input
              type="radio"
              name={`discount_${field}`}
              checked={config.enabled === "no"}
              onChange={() => this.setDiscountConfig(field, { enabled: "no", percent: "" })}
            />{" "}
            No
          </label>
        </div>
        {config.enabled === "yes" ? (
          <input
            ...
            onChange={(e) => this.setDiscountConfig(field, { percent: e.target.value })}
          />
        ) : null}
      </div>
    );
  ...
  handleSetPlan = (values, item) => {
    ...
    const adjustedFees = item && item.oneTime && item.termI && item.termII
      ? item
      : this.getAdjustedFeeDetails(item);
    ...
    if(values.term==1){
      postData = {
        ...
        room_rent: adjustedFees.oneTime.room_rent,
        cultural_fees: adjustedFees.oneTime.cultural_fees,
        caution_deposit: adjustedFees.oneTime.caution_deposit,
        addmission_fee: adjustedFees.oneTime.addmission_fee,
        admisson_kit: adjustedFees.oneTime.admisson_kit,
        meal_t1: adjustedFees.termI.meal_price_t1,
          ...
          <section className="content-header">
            ...
            <div className="card card-m-l11">
              ...
              <div className="row1">
                ...
                const discountedFees = this.getAdjustedFeeDetails(item);
    return (
                ...
                <td>
                <b>
                {discountedFees.total_price.toLocaleString(
                  "en-IN",
                  {
                    maximumFractionDigits: 0,
                    style: "currency",
                ...
                type="button"
                onClick={(e) => {
                  this.submitAlert(this.state.formValues, discountedFees);
                }}
                style={{
